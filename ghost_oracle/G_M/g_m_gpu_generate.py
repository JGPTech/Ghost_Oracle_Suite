...
ctrl, a, and b are INDEPENDENT per shot (the ghost ancillas commute with the
CSWAP+H sequence). The CUDA kernel samples from this closed form directly —
no statevector simulation, three curand_uniform draws per shot.

The sampling MUST go through ghost_ghz_sample. A marginal-only check on
P(ctrl=0) cannot tell a correct joint from a corrupted one (a wrong joint can
reproduce the right marginal), so this builder also verifies the per-bucket
conditional P(ctrl=0 | a, b) — which is what the projection channel reads.
...
# Per-bucket independence check: minimum shots in a bucket before its empirical
# P(ctrl=0) is trusted (shot-noise floor for n is ~0.5/sqrt(n)).
BUCKET_MIN_COUNT = 64

# Repo-root paths. This file lives at <repo>/gpu.py.
...
def build_base(matrix_size=4, n_shots=4096, seed=None, verbose=False, kernel=None):
    ...
    Sampling goes through the ghost_ghz_sample CUDA kernel (ghost_kernel.cu,
    Section 5): three independent per-shot draws (ctrl, a, b), with the GHZ
    structure giving a1 == a2 and b1 == b2. ctrl is independent of (a, b).
    ...
    data : dict
        ...
        Plus diagnostic keys (stripped before on-disk write by CLI):
            max_p0_residual     : float  marginal P(ctrl=0) vs T3, ~shot-noise
            max_bucket_residual : float  per-bucket P(ctrl=0|a,b) vs T3 — the
                                         independence check; a corrupted joint
                                         blows this up while max_p0_residual
                                         stays small
            max_anti            : float  ghost anti-correlation, 0 exactly
    ...
    # ---------- GHZ sampler (ghost_kernel.cu, Section 5) ----------
    # One block per tile; threads stride the shot range. Each shot draws
    # (ctrl, a, b) from three independent curand_uniform calls, so ctrl is
    # independent of the ghost bits — exactly what the circuit produces.
    # Reproducible given (seed, tile, shot) via the per-shot subsequence.
    d_angles_a = cp.asarray(tile_angles_a, dtype=cp.float32)
    d_angles_b = cp.asarray(tile_angles_b, dtype=cp.float32)
    ...
    ctrl_host = d_ctrl.get()
    ghost_host = d_ghost.get()
    ...
    for t in range(num_tiles):
        ...
        # Per-bucket independence check: ctrl is independent of (a, b), so
        # P(ctrl=0 | a, b) must equal expected_p0 in EVERY populated bucket.
        # This is what projection_channel reads. The marginal emp_p0 above is
        # blind to a corrupted joint; this is not.
        a_bits = ghost[:, 0]
        b_bits = ghost[:, 2]
        for av in (0, 1):
            for bv in (0, 1):
                bmask = (a_bits == av) & (b_bits == bv)
                nb = int(bmask.sum())
                if nb >= BUCKET_MIN_COUNT:
                    p0_bucket = float((ctrl[bmask] == 0).mean())
                    max_bucket_residual = max(
                        max_bucket_residual,
                        abs(p0_bucket - diag["expected_p0"]),
                    )
    ...
    data["max_bucket_residual"] = max_bucket_residual
...
def main():
    ...
    max_bucket_residual = data.pop("max_bucket_residual")
    ...
    bucket_floor = 0.5 / math.sqrt(BUCKET_MIN_COUNT)

    print("\n  Empirical sanity:")
    print(
        f"    Max |P(ctrl=0) - T3|         : {max_p0_residual:.2e}   "
        f"(marginal; shot-noise floor ~ {shot_floor:.2e})"
    )
    print(
        f"    Max |P(ctrl=0|a,b) - T3|     : {max_bucket_residual:.2e}   "
        f"(per-bucket independence; floor ~ {bucket_floor:.2e} at n={BUCKET_MIN_COUNT})"
    )
