# Ghost Oracle Suite — G_M Process Record

This document records the research and engineering trajectory of the **G_M (Ghost Metric)** operator family within the Ghost Oracle Suite, from initial probes through the five-way benchmark and the optimized capstone that followed it. It exists so that any future contributor — human or AI agent — can pick up the G_M work with full context.

The **S_M (Syndrome Metric)** family — the repetition-code / syndrome-spacetime operator, the S_M→TSP projector, and the S_M→token-retrieval bridge — is recorded in its own process record. The repository is split into `ghost_oracle/G_M/` and `ghost_oracle/S_M/` because the two operators have different natural domains and deserve separate records.
...
> **[Note added during the post-five-way consolidation: this "final" benchmark is superseded by the optimized capstone `G_M_final_benchmark.py`. See Part 17. The d=256 result above stands as a snapshot; the harder default regime and the scale/control stages are in Part 17.]**

---

## Part 14 — Updated open questions

These supersede the open questions from the original Part 10 of this record. *(Further extended and partially resolved in Part 20.)*
...
> **[Superseded — see Part 18 / Part 19.]** The `ctrl = 0 when a==b` rule recorded here is itself joint-wrong: it reproduces the correct *marginal* `P(ctrl=1)` but corrupts the *per-bucket conditional* `P(ctrl=0 | a,b)` that the projection channel reads. The real fix wires the `ghost_ghz_sample` kernel (independent `ctrl` draw) and adds a per-bucket independence check. Every GPU base generated under the `ctrl = 0 when a==b` sampler is corrupted at the per-bucket level and must be regenerated.
...
The sections below continue the record after the five-way benchmark, during the operator-folder consolidation of the G_M family. Parts 1–16 above are preserved as the frozen historical record; the parts below are the current state.

---

## Part 17 — Consolidation: the optimized capstone benchmark

The repository was reorganized so each operator family is a self-describing folder: the three core scripts (QPU generator, GPU generator, benchmark) plus `README.md`, `process_record.md`, and `probe/` `example/` `data/` subfolders. The G_M scripts were consolidated to fit this.

### File consolidation

**`qpu.py` + `dump.py` → `gm_qpu.py`.** The submit and dump scripts merged into one file with two subcommands, `submit` and `dump`. They stayed as two entry points rather than one linear run because the QPU is asynchronous: `submit` fires the job and returns the ID without blocking; `dump` pulls it later, once complete, and is the line that writes the frozen base into `data/` — the freeze step. Everything preserved verbatim: the `find_qubit_clusters` readout-error sort fallback (the `i in target['measure']` comparison that's always False, collapsing the sort to ascending qubit-index order — intentional, the layout the whole probe series was calibrated under), `ANGLE_SCALE = 1.05`, `STAGGER_DT = 72`, `EXCLUDED_QUBITS`, the XY4 dynamical-decoupling injection, the `bs[::-1]` ghost-bit reversal into `[a1, a2, b1, b2]`, the contiguous re-store as `ctrl_tile{out_t}`, both `QiskitRuntimeService` instantiation styles, and the `data/job_<JOB_ID>.npz` filename. The only changed string is the post-submit "next step" hint, which now points at `gm_qpu.py dump` instead of the removed `dump.py`.

**`projection_benchmark.py` → `G_M_benchmark.py`.** A rename only; the headline projection benchmark is unchanged.

### `G_M_final_benchmark.py` — the optimized capstone

A merge of the two capstone scripts, which were not duplicates but two different benchmarks sharing helpers:

- `final_benchmark_5way.py` — the **verification** story: gaussian keys + jitter + same-dim spike, the five named paths (CUBLAS / TIED / GEO / QPROJ / GPROJ), per-base calibration, the Flash-Squelch sig/spk certificate, the agreement metric, a JSON summary.
- `auto_oracle.py` — the **scale + controls** story: clustered semantic environments up to M=1M, top-K softmax *mixed* components, recall@K + MRR, and the two negative-control probes (load-bearing real-vs-perm-vs-uniform, and the separation spectrum).

**Engine.** One compile of `ghost_kernel.cu + megakernels_2d.cu`. The inline `FINAL_KERNELS` block from `final_benchmark_5way` was dropped entirely. The production path uses the 2D megakernels exclusively: `geo_megakernel_2d` (probes/sweep), `proj_megakernel_2d` (calibration/probes), `tied_megakernel_2d` (the five-way verify — geom + proj + per-query agreement in one launch).

**The key correctness point of the engine swap:** the 2D megakernels fold the phase-lift + cos *into* the global→shared transfer (`lift_and_cos`), so they consume **raw embeddings**, not the host-side `theta` arrays the old `fb_compute_all_channels` wanted. So `phase_lift_perdim` is gone everywhere — verify, calibration, and probes all feed raw embeddings now. That is a real simplification the swap bought, not a corner cut. And because the projection half of `tied_megakernel_2d` is bit-identical math to `proj_megakernel_2d` (both call `projection_from_cos`), a threshold picked during calibration applies exactly to the verify, and the metric used to calibrate is the metric the verify reports.

Three stages, flag-driven: verify (default), `--sweep`, `--probe`.

### Calibration objective split

The first version of the merge used one certificate-scored calibration routine for everything. That was wrong, and the sweep exposed it: components calibrated by the sig/spk certificate would sometimes be attack-clean but scramble semantic-neighbourhood ordering, collapsing to ~0% at scale, while the table still printed them as weight-1.00 winners.

The fix is two selectors, because the two tasks do different things:

- **Verify** scores under a threshold and reports sig/spk → `calibrate_candidates` (the certificate; clean-first, then signal fraction). Single best component on the gaussian-jitter task.
- **Sweep / probes** are pure argmax retrieval that throw the threshold away → `calibrate_recall1` (rank by the same argmax recall@1 the sweep runs). A component is only selected if it retrieves, so the certificate's "attack-clean but scrambles order" failure mode cannot pick a non-retriever. Sweep uses the top-K softmax mix; probes use the single best.

### Harder verify defaults

At the inherited d=256 / magnitude=50, cuBLAS hit 100% top-1 — nothing separated, so the capstone proved nothing (the same "too easy" condition the separation probe exists to catch). The defaults moved to the mid-dimensional regime **d=64 / magnitude=200**.

The arithmetic: cuBLAS's true-match score scales like `d` (≈64), while the spike contributes `q_i[k_bad] · magnitude`. At magnitude 200 that term swamps 64 whenever `q_i[k_bad] > 0`, so cuBLAS falls toward a structural ~50% floor for this single-shared-dim attack — half the queries see the spike as positive and get fooled, the other half see it as negative and the true match wins easily. Observed: cuBLAS ~57% top-1 (a hair above the floor because at d=64 the honest dims still pull some weight), spk jumping to ~0.42; GEO/TIED holding ~100% across all bases; QPROJ/GPROJ ~100% / ~99.8%. That is the separation the capstone needs. `magnitude` touches only the verify and its gaussian calibration; the semantic sweep keeps its own magnitudes.

(GEO/TIED top-1 reads identically across all bases on purpose — geometry never touches the bucket counts, so only QPROJ/GPROJ are base-dependent. That identical GEO row is a correctness tell, not a bug.)

---

## Part 18 — The `gpu.py` sampler, re-corrected (the marginal-blind joint bug)

This is the most important post-five-way correction, and it supersedes the `gpu.py` fix recorded in Part 11 / Part 15.

### The trigger

In the consolidated benchmark's sweep, exactly one GPU base in four kept collapsing to ~0% recall while the others retrieved at 100%. The failing base *moved with the seed* across runs (seed134, then seed392) — which ruled out a position-fixed benchmark bug and a uniform sampler bug, and pointed at a per-base data defect of roughly 1-in-N incidence. The collapse fingerprint was always ≈0.78% ≈ 8/1024 — floor-of-random, meaning calibration tried every tile×mask and the best still retrieved nothing.

### The diagnosis

Reproducing the generator's sampling logic in numpy showed the shipped vectorized sampler did:

```python
ctrl = where(a_bit == b_bit, 0, fair_coin)
```

— which is exactly the Probe-20 "fix." This correlates `ctrl` with whether the ghost bits agree. The real circuit, and the `ghost_ghz_sample` kernel that was sitting in `ghost_kernel.cu`, draws `ctrl` as an **independent** Bernoulli with `P(ctrl=1) = 0.5·(1 − swap)`, `swap = ca·cb + sa·sb`.

The two share the same *marginal* `P(ctrl=1)` — which is precisely why this shipped. `gpu.py --verify` only checked the marginal `P(ctrl=0)` against the T3 target, and the marginal matched to three decimals on every tile for both the buggy and correct samplers. The marginal-only control is provably blind to this bug.

But `projection_channel` does not read the marginal. It reads the **per-bucket conditional** `P(ctrl=0 | a, b)`, and that was corrupted:

```
correct:  P(ctrl=0 | a,b) ≈ expected_p0   for every (a,b) bucket
buggy:    P(ctrl=0 | a,b) = 1.0  in diagonal buckets (a==b)
                          = 0.5  in off-diagonal buckets (a≠b)
```

The buggy projection surface is a checkerboard with no dependence on the actual angles. The "too many zeros" originally noticed was every diagonal bucket's `ctrl=1` cell being *exactly* 0.

### The root cause

`build_base` compiled `ghost_ghz_sample`, accepted it as the `kernel=` argument, and **never called it** — the correct kernel was dead code, replaced by the buggy inline cupy sampler (presumably for speed). The careful three-independent-draw kernel was right there the whole time.

### The fix

Wire the actual `ghost_ghz_sample` launch (one block per tile, threads stride the shots, three independent `curand_uniform` draws per shot, so `ctrl` is independent of the ghost bits). Schema, dtypes, and `[a1, a2, b1, b2]` column order unchanged → existing dumps stay byte-compatible.

### The control added

A **per-bucket independence check** in `--verify`/diagnostics: `max |P(ctrl=0 | a,b) − expected_p0|` over populated buckets (n ≥ 64). On clean data this sits near the shot-noise floor (~0.06 at n=64); the old buggy data would have read ~0.27. It is now printed alongside the marginal residual, and the two disagreeing is the signature of a broken joint. This is the control that catches the class of bug the marginal-only check cannot. After regeneration, per-bucket residuals across all bases sat at the floor — the generator is clean.

### The residual finding: degenerate tiles

After the generator fix, one base (seed392) *still* calibrated and retrieved at floor (0.78%). Inspecting the actual base file showed it was structurally perfect — per-bucket conditionals on target, residual 6.5e-02 at the floor. The problem was not the data; it was `P(b=1)`.

The B family `[1.0, 0.8, 0.4, 0.1]` maps its smallest entry (0.1) through `data_to_angles` to a near-zero rotation, so `sin²(b/2) ≈ 0.006`. Tiles in the b=0.1 column (tiles 3, 7, 11, 15) therefore have `b` pinned to 0 almost always; tile 7's `(a=1,b=1)` bucket held n=2 shots. With `b` effectively constant, the projection surface for those tiles has almost no dynamic range — they are **structurally weak retrievers regardless of shot correctness**. seed392's shot noise floated such a tile (t7) to the top of the recall@1 ranking, and with no floor, calibration handed it over at weight 1.00.

### The fix for the symptom: calibration floor

A recall@1 floor (`CALIB_MIN_R1`, default 0.50, `--cal-min-r1` override) on the sweep/probe selector. A base whose best component can't clear the floor is announced (`NO USABLE COMPONENT`, with the failing tile/mask and the actual recall) and excluded from the sweep and probes, rather than slipped in as a weight-1.00 component that prints 0%. Same discipline as the per-bucket check: don't let a thing that didn't pass present as if it did. (Probe B also guards against an all-excluded set.)

### The cause, deferred

The B-family bottoming near zero is an experiment-design issue, not a code bug. The clean fix is widening the minimum angle so `sin²(b/2)` doesn't collapse, but that touches the canonical pipeline (`gm_qpu.py` and `gpu.py` both build from the B vector) and invalidates every existing base against the current constants — a regenerate-everything decision left to the maintainer. Recorded as an open question (Part 20).

### The meta-lesson

This is the **second** time the project was bitten by a marginal-blind control. The first was the original Probe 12 "quantum advantage" claim, which was measuring a buggy classical control. This time the Probe-20 `gpu.py` fix got the *marginal* right — which killed the visible bimodal-lock symptom and made the fix look complete — while leaving the *joint* corrupted in a way that surfaced much later as intermittent base collapse. The control that catches it has to check the quantity the consumer actually reads (the per-bucket conditional), not the summary statistic that happens to be easy to check (the marginal).

---

## Part 19 — Known issues (updates Part 15)

The Part 15 entries stand as the historical snapshot. Current state:

### `gpu.py` sampler — corrected at the joint level (was: corrected at the marginal level)
The Probe-20 `ctrl = 0 when a==b` rule reproduces the correct marginal but corrupts the per-bucket joint that `projection_channel` reads. The current `gpu.py` launches `ghost_ghz_sample` (independent `ctrl`) and verifies the per-bucket conditional. **Every GPU base generated before this fix — including the ones under the Probe-20 sampler — is corrupted at the per-bucket level and must be regenerated.** QPU `job_*.npz` bases are real hardware and are unaffected.

### Degenerate B-column tiles
Tiles in the b=0.1 column are structurally weak retrievers (near-zero `sin²(b/2)`), regardless of shot correctness. The calibration recall@1 floor excludes bases that can only field such tiles. The underlying angle redesign is deferred (Part 20).

### Calibration is two-objective
Certificate (sig/spk) for the verify; recall@1 for the sweep/probes. Using the wrong objective for a task silently degrades it — certificate-selected components can collapse at scale. Don't unify them.

### Verify regime must separate cuBLAS
At high d / low magnitude cuBLAS ties at 100% and the capstone proves nothing. The default d=64 / magnitude=200 is chosen to drive cuBLAS toward its ~50% single-dim-attack floor. If the regime is softened, re-check that cuBLAS still collapses.

### Carried forward unchanged
Probe 8.2 alternation loop broken; Probe 9 Demo 2 (fixed in 9.1); Probe 10 superseded by 10.1; Probe 8.4 shot-count artifacts; Probe 16 Golden Mask retracted; Probe 12 quantum-advantage framing retracted; phase-lift design informal (now removed from the benchmark path entirely, since the megakernels lift internally).

---

## Part 20 — Open questions (updates Part 14)

1. **B-family angle redesign.** Widen the minimum B angle so no tile is degenerate (e.g. floor `sin²(b/2)` around 0.05, or replace `[1.0, 0.8, 0.4, 0.1]` with something like `[1.0, 0.7, 0.4, 0.15]`). This removes the structurally-weak tiles at the source instead of catching them with the calibration floor. It is a canonical-pipeline change (both generators build from the B vector) and invalidates every existing base, so it's a deliberate regenerate-everything decision, not a quiet edit.

2. **Calibration floor heuristic.** `CALIB_MIN_R1 = 0.50` is an absolute cutoff. A better floor would be relative — "must beat cosine on the calibration env" — so it adapts to harder regimes where 50% isn't the right bar.

3. **Sweep challenge dial.** The semantic sweep runs on unit-normalized embeddings, where outlier magnitude barely bites (a spiked key normalizes to near-one-hot regardless of magnitude). The real dial for making the sweep harder is lowering `--sweep-d`; the SWEEPS magnitude column is mostly inert. Characterize where the semantic attack actually engages.

4. **Re-measure Probe 12 against the corrected generator.** The substrate-equivalence conclusion survives, but the exact GPU PROJ gap (0.12119) was measured on the joint-wrong Probe-20 sampler. Re-run against the `ghost_ghz_sample`-correct generator for a final number.

5. **Carried forward from Part 14:** real LLM embeddings for the G_M five-way; the cuBLAS performance gap and whether a tensor-core-friendly G_M closes it; the ~80% QPU-agreement variance across calibrations; the mask infrastructure's load-bearing operating point; the geometry-only production shipping target.

---

## Part 21 — Closing note (extends Part 16)

The post-five-way work reinforced the pinned lesson and gave it a sharper form.

Twice now the project was misled by a control that checked a *summary* of the data rather than the part the consumer actually uses. The original Probe 12 checked a retrieval gap computed on a buggy classical baseline. The Probe-20 `gpu.py` fix checked the marginal `P(ctrl=0)` while the projection channel quietly fed on the per-bucket conditional. Both controls passed. Both were blind. In both cases the fix was the same shape: build the control around the quantity downstream actually reads, and have a thing that didn't pass announce itself instead of presenting as a result.

That is the standard the G_M family keeps:

```text
Build the thing that should work.
When it does something else, do not throw it away — ask what it actually computed.
Check the quantity the consumer reads, not the summary that's easy to check.
If a base, component, or run didn't pass, make it announce itself.
Freeze it, control it, scramble it, document it. In the open. CC0.
```
